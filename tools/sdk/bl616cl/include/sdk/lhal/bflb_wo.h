 ...
 * @brief Initialize wo.
 *
 * @note For BL616CL, this API resets wo clock divider to 1.
 * @note If a custom clock divider is required, call bflb_wo_set_clk_div() after bflb_wo_init().
 *
 * @param [in] dev device handle
...
/**
 * @brief Set wo clock divider. This API is only valid for BL616CL.
 *
 * @note Clock divider defaults to 1.
 * @note bflb_wo_init() resets divider to 1, so call this API after bflb_wo_init().
 * @note If clock divider is set to a value other than 1, the first waveform generated by wo may have an incorrect proportion.
 *
 * @param [in] dev device handle
 * @param [in] clk_div clock divider value, 0 will be treated as 1
 */
