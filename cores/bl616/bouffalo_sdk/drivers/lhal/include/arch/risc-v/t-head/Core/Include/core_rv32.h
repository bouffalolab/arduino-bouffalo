...
*/
__STATIC_INLINE void csi_dcache_invalid_range(void *addr, int32_t dsize)
{
#if (__DCACHE_PRESENT == 1U)
...
/**
  ...
  \param[in]   dsize   size of memory block (in number of bytes)
*/
__STATIC_INLINE void csi_dcache_clean_range(void *addr, int32_t dsize)
{
#if (__DCACHE_PRESENT == 1U)
    int32_t op_size = dsize + (uint32_t)addr % 32;
    uint32_t op_addr = (uint32_t)addr & CACHE_INV_ADDR_Msk;
...
*/
__STATIC_INLINE void csi_dcache_clean_invalid_range(void *addr, int32_t dsize)
{
#if (__DCACHE_PRESENT == 1U)
    int32_t op_size = dsize + (uint32_t)addr % 32;
    uint32_t op_addr = (uint32_t)addr & CACHE_INV_ADDR_Msk;
